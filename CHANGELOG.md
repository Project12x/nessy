...
### Added
- **NSF engine core (Phase B.1)** — parses NSF/NSFe, runs the ripped 6502 via the restored nes_cpu (INIT + self-clocking PLAY), and plays a synthetic NSF non-silently in the test suite. Engine lives in `src/nsf/NsfEngine`; not yet wired to the plugin/UI (B.2/B.3).
