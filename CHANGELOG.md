...
### Added
- **MMC5 + Sunsoft 5B synth voices (Phase C.2)** — MMC5 (2 pulses, 4 duty cycles) and Sunsoft 5B / FME7 (3 square tones) are now MIDI-playable through the unified C.1 voice pool at full feature parity with the existing channels: per-chip enable params (`mmc5Enable`, `sunsoft5bEnable`), MacroEngine (vibrato/decay/arp/duty-sweep/stab), and portamento. New APVTS params: `mmc5Pulse1Duty`, `mmc5Pulse2Duty`, and 5 per-channel macro-preset selectors (`macroMmc5P1`, `macroMmc5P2`, `macroFme7A`, `macroFme7B`, `macroFme7C`). Channel count grew 8 → 13. New chip groups default off (synth unchanged until enabled). 5B uses the AY PSG period mapping; A4 pitch was empirically verified correct (440 Hz) during code review. UI controls land in Phase D (test via a host's generic parameter view for now).
...
- **VRC6 channels gained volume-macro support (Phase C.2 side effect)** — the channel-aware `applyMacroVolume`/`applyMacroDuty` helpers introduced for MMC5/5B parity also route volume macros to VRC6 Pulse 1/2/Saw for the first time (previously absent). Additive only; no existing 2A03 behaviour changed.
- **`midiToPeriod` is now chip-aware; VRC6 pitch clamp widened to 12-bit (Phase C.2)** — MMC5 channels (ids 8/9) correctly use the 11-bit 2A03 clamp (2047) rather than the 12-bit VRC6 clamp. VRC6 Saw pitch clamp was widened to 12-bit (4095) as part of the chip-aware refactor.
