#if defined(_WIN32)
#include <windows.h>
// TRACE is an MFC macro (afx.h). nsfplay's debugout.h maps DEBUG_OUT to TRACE
// on MSVC, but nessy_tests is a non-MFC build, so define a no-op stub here
// before debugout.h is included transitively via nsf.h -> soundData.h.
#ifndef TRACE
#define TRACE(...) ((void)0)
#endif
#else
...
#include <string.h>
#include "nsf.h"
// NSF_ENABLE_FILEREADER: file-reader and playlist paths are not needed in the
// nessy_tests build (tests call Load(buf,size) directly).  Guard out the two
// pull-in headers so this translation unit remains self-contained.
#ifdef NSF_ENABLE_FILEREADER
#include "../../fileutil.h"
extern "C"
{
#include "pls/ppls.h"
